...
<div class="j-wrapper j-wrapper-640">
     <form class="j-pro" novalidate="" id="formajax" action="<?= base_url('utangpiutang/update/'.$dataEdit['id_utang']) ?>" autocomplete="off">
        <div class="j-content">
            ...
            <div class="j-unit">
                <div class="checkbox-fade fade-in-primary mt-0">
                    <label>
                        <?php 
                            $tempoCheck = ''; $tempoAria = 'true'; $tempoClass = 'class="collapsed"'; $tempoCollapse = '';
                            if($dataEdit['tanggal_tempo'] != null) { $tempoCheck = 'checked'; $tempoAria = 'false'; $tempoClass = ''; $tempoCollapse = 'show';}
                        ?>
                        <input <?= $tempoCheck.$tempoClass ?> type="checkbox" value="true" name="jatuh_tempo" data-toggle="collapse" href="#collapseTempo" aria-expanded="<?= $tempoAria ?>" aria-controls="collapseTempo">
                        <span class="cr"><i class="cr-icon icofont icofont-ui-check txt-primary"></i></span>
                        <span class="text-inverse">Jatuh Tempo</span>
                    </label>
                </div>
            </div>
            <div class="j-row panel-collapse collapse in <?= $tempoCollapse ?>" id="collapseTempo" role="tabpanel" aria-labelledby="headingTempo">
                <div class="j-span7 j-unit">
                    <label class="j-label">Tanggal Jatuh Tempo <small class="text-danger">*</small></label>
                    <div class="j-input">
                        <label class="j-icon-right" for="date_from">
                            <i class="fa fa-calendar"></i>
                        </label>
                        <input type="text" class="date_from" name="tanggal_tempo" readonly="" value="<?=$dataEdit['tanggal_tempo'] != null ? date('Y-m-d',strtotime($dataEdit['tanggal_tempo'])) : ''?>">
                    </div>
                </div>
                <div class="j-span5 j-unit">
                    <label class="j-label">Jam Jatuh Tempo <small class="text-danger">*</small></label>
                    <div class="j-input" >
                        <label class="j-icon-right" for="date_to">
                            <i class="fa fa-clock-o"></i>
                        </label>
                        <input type="text" class="jam" name="jam_tempo" value="<?=$dataEdit['tanggal_tempo'] != null ?  date('H:i',strtotime($dataEdit['tanggal_tempo'])) : ''?>">
                    </div>
                </div>
            </div>
            <div class="j-unit">
                <label class="j-label">Klien  <small class="text-danger">*</small></label>
                <div class="input">
                    <input type="text" name="klien" value="<?=$dataEdit['klien']?>">
                </div>
            </div>
            <div class="j-unit">
